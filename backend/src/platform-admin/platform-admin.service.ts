...
export class PlatformAdminService {
  constructor(
    ...
    private institutionAdmin: InstitutionAdminService,
  ) {}

  listInstitutionPendingApprovals() {
    return this.institutionAdmin.listPendingApprovals();
  }

  approveInstitutionRequest(requestId: string) {
    return this.institutionAdmin.approveRequest(requestId);
  }
  ...
  async invite(dto: {
    ...
    const exists = await this.prisma.platformAdmin.findFirst({
    ...
    const admin = await this.prisma.platformAdmin.create({
      data: {
        ...
        created_by_super_admin: dto.createdBySuperAdminId,
        onboarding_completed: false,
      },
    });

    const magicLink = `${FRONTEND_URL}/platform-admin/auth/magic?token=${magicToken}&email=${encodeURIComponent(email)}`;
    ...
    return {
      ...
      message: 'Platform admin invited. Recovery code shown once — save it. Magic link sent to primary email.',
    };
  }
  ...
  private async sendMagicLinkEmail(email: string, link: string) {
    const isDev = process.env.NODE_ENV !== 'production';
    if (isDev) {
      console.log(`[Platform Admin] Magic sign-in link for ${email}: ${link}`);
  ...
  async listPendingApprovals() {
    return this.prisma.platformAdminApprovalRequest.findMany({
  ...
  async approveRequest(requestId: string) {
    const req = await this.prisma.platformAdminApprovalRequest.findUnique({
      where: { id: requestId },
    });
    if (!req || req.status !== 'pending') throw new NotFoundException('Request not found');

    await this.prisma.platformAdminApprovalRequest.update({
      where: { id: requestId },
      data: { status: 'approved' },
    });

    return { approved: true, platformAdminId: req.platform_admin_id };
  }
  ...
  async getSigninSession(sessionId: string) {
    const session = await this.prisma.platformAdminSigninSession.findUnique({
