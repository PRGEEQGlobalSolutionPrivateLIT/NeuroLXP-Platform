...
import { InstitutionAdminJwtGuard } from './institution-admin-jwt.guard';

type AuthRequest = { user: { id: string; email: string } };

@Controller('api/institution-admin')
export class InstitutionAdminProfileController {
  constructor(private prisma: PrismaService) {}

  @Get('me')
  @UseGuards(InstitutionAdminJwtGuard)
  async getProfile(@Req() req: AuthRequest) {
    const admin = await this.prisma.institutionAdmin.findUnique({ where: { id: req.user.id } });
  ...
  async updateProfile(
    ...
    const admin = await this.prisma.institutionAdmin.update({
