...
@Injectable()
export class InstitutionAdminSigninService {
  constructor(
  ...
  async primarySignin(identifier: string, password: string) {
    ...
    const admin = await this.prisma.institutionAdmin.findFirst({ where });
  ...
  async sendPrimaryOtp(sessionId: string, method: 'email' | 'phone') {
    const session = await this.institutionAdminService.getSigninSession(sessionId);
    const admin = await this.institutionAdminService.getAdmin(session.institution_admin_id!);
  ...
  async verifyRecovery(sessionId: string, code: string) {
    ...
    await this.prisma.institutionAdminSigninSession.update({
  ...
  async requestPlatformAdminApproval(sessionId: string) {
    const session = await this.institutionAdminService.getSigninSession(sessionId);
    const admin = await this.institutionAdminService.getAdmin(session.institution_admin_id!);

    const existing = await this.prisma.institutionAdminApprovalRequest.findFirst({
      where: { institution_admin_id: admin.id, status: 'pending' },
    });
    if (existing) {
      return { requestId: existing.id, status: 'pending', message: 'Approval already requested' };
    }

    const req = await this.prisma.institutionAdminApprovalRequest.create({
      data: {
        institution_admin_id: admin.id,
        session_id: sessionId,
        status: 'pending',
    ...
    return {
      requestId: req.id,
      status: 'pending',
      message: 'Platform Admin approval requested. You will be notified when approved.',
    };
  }
  ...
  async checkApprovalAndSignin(sessionId: string) {
    ...
    const approved = await this.prisma.institutionAdminApprovalRequest.findFirst({
      where: { institution_admin_id: admin.id, status: 'approved' },
      orderBy: { updated_at: 'desc' },
    });

    if (!approved) {
      return { approved: false, message: 'Waiting for Platform Admin approval' };
    }

    return this.completeSignin(sessionId, admin.id);
  }

  private async completeSignin(sessionId: string, institutionAdminId: string) {
    const session = await this.institutionAdminService.getSigninSession(sessionId);
    const draft = this.getSigninDraft(session);
    const admin = await this.institutionAdminService.getAdmin(institutionAdminId);
    const tokens = await this.auth.createSession(institutionAdminId);
