...
export function InstitutionAdminSigninWizard() {
  ...
  const sendPrimaryOtp = () =>
    run(async () => {
      ...
      try {
        const { data } = await institutionAdminApi.sendOtp(sessionId, primaryChannel, 'primary');
        setOtpSent(true);
  ...
  const sendAltOtp = () =>
    run(async () => {
      ...
      try {
        const { data } = await institutionAdminApi.sendOtp(sessionId, altChannel, 'alt');
        setOtpSent(true);
        if (data.devOtp) setDevOtp(data.devOtp);
        const rem = data.sendsRemaining ?? 0;
  ...
  const titles: Record<Phase, { title: string; subtitle: string }> = {
    credentials: { title: 'Institution Admin Sign In', subtitle: 'Email, phone, or User ID with password' },
    ...
    approval: { title: 'Platform Admin Approval', subtitle: 'Final step — request approval to access dashboard' },
  };
      ...
      case 'approval':
        return (
          <NeumorphicCard className="!p-4 text-sm">
            {approvalRequested ? (
              <p>Waiting for Platform Admin to approve your access. Click below to check status.</p>
            ) : (
              <p>Request Platform Admin approval to complete sign-in.</p>
            )}
          </NeumorphicCard>
        );
    }
    ...
    if (phase === 'credentials') {
      return (
        <NeumorphicButton
          ...
          onClick={() =>
            run(async () => {
              const { data } = await institutionAdminApi.primarySignin(identifier, password);
      ...
      return (
        <NeumorphicButton
          ...
          onClick={() =>
            run(async () => {
              if (!primaryChannel) return;
              const { data } = await institutionAdminApi.verifyOtp(sessionId, primaryChannel, otp, 'primary');
      ...
      return (
        <NeumorphicButton
          ...
          onClick={() =>
            run(async () => {
              if (!altChannel) return;
              const { data } = await institutionAdminApi.verifyOtp(sessionId, altChannel, otp, 'alt');
    ...
    if (phase === 'totp') {
      return (
        <NeumorphicButton
          ...
          onClick={() =>
            run(async () => {
              const { data } = await institutionAdminApi.totp(sessionId, totpCode);
    ...
    if (phase === 'recovery') {
      return (
        <NeumorphicButton
          ...
          onClick={() =>
            run(async () => {
              const { data } = await institutionAdminApi.recovery(sessionId, recoveryCode);
              if (data.matched) {
                if (data.newRecoveryCode) {
                  sessionStorage.setItem(INSTITUTION_RECOVERY_STORAGE_KEY, data.newRecoveryCode);
                }
                neoToast.info('Recovery code accepted — request Platform Admin approval');
                await institutionAdminApi.requestApproval(sessionId);
      ...
      return (
        <>
          ...
          <NeumorphicButton
            ...
            onClick={() =>
              run(async () => {
                const { data } = await institutionAdminApi.checkApproval(sessionId);
                if (data.accessToken) finishLogin(data);
                else neoToast.info('Still waiting for Platform Admin approval');
              })
            }
          >
