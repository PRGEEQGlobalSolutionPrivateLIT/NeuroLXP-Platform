...
export function InstitutionAdminOnboardingWizard() {
  ...
  useEffect(() => {
    if (!sessionId) router.replace('/institution-admin/auth/signin');
  }, [sessionId, router]);
      ...
      footer={
        <NeumorphicButton
          ...
          onClick={() => {
            if (phase === 'password') {
              run(async () => {
                const { data } = await institutionAdminApi.resetPassword(sessionId, password);
                setUserId(data.userId);
                neoToast.success(`Password set. User ID: ${data.userId}`);
                const { data: totp } = await institutionAdminApi.totpSetup(sessionId);
                setQrCode(totp.qrCodeDataUrl);
                setPhase('totp');
              });
            } else if (phase === 'totp') {
              run(async () => {
                await institutionAdminApi.totpVerify(sessionId, totpCode);
                neoToast.success('Authenticator verified');
                setPhase('alt_contact');
              });
            } else {
              run(async () => {
                await institutionAdminApi.setAltContact(sessionId, {
                  altEmail,
                  altPhone,
                  emailVerified,
                  phoneVerified,
                });
                const { data } = await institutionAdminApi.completeOnboarding(sessionId);
