...
import { superAdminApi, SuperAdminProfile } from '@/lib/superadmin-api';
import { setStoredAvatar } from '@/superadmin/lib/sa-avatar';
...
export function SuperAdminProfilePage() {
    ...
    superAdminApi
      ...
      .then(({ data }) => {
        ...
        setForm({
          ...
          approverDesignation: data.approverDesignation,
  ...
  const save = async () => {
    setLoading(true);
    try {
      await superAdminApi.updateProfile(form);
  ...
  const onPhotoPick = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      setStoredAvatar(user?.userId, dataUrl);
  ...
  const removePhoto = () => {
    setStoredAvatar(user?.userId, null);
    ...
    <div className="space-y-6">
      ...
      <div className="neo-card p-6 md:p-8">
        <div className="flex flex-col items-center gap-4 border-b border-[var(--neo-shadow-dark)]/10 pb-6 sm:flex-row sm:items-start">
          <SuperAdminAvatar userId={user?.userId} name={profile.fullName} size="md" />
        ...
        {editing ? (
          <div className="neo-form-stack mt-6">
            <div className="neo-inset rounded-2xl p-5">
              ...
              <div className="mt-4 flex justify-center">
                <SuperAdminAvatar userId={user?.userId} name={form.fullName || profile.fullName} size="md" />
              </div>
            </div>
            ...
            <NeumorphicInput
              ...
              onChange={(e) => setForm((f) => ({ ...f, approverDesignation: e.target.value }))}
            />
            <NeumorphicInput
            ...
            <NeumorphicInput
              ...
              value={form.approverPhone}
              onChange={(e) => setForm((f) => ({ ...f, approverPhone: e.target.value }))}
            />
          ...
          <dl className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Approver" value={profile.approverName || '—'} />
            <Field label="Approver designation" value={profile.approverDesignation || '—'} />
            <Field label="Approver email" value={profile.approverEmail || '—'} />
            <Field label="Signup completed" value={profile.signupCompleted ? 'Yes' : 'No'} />
          </dl>
        )}
      </div>
    </div>
