...
import { platformAdminApi, PlatformAdminProfile } from '@/lib/platform-admin-api';
import { setStoredPaAvatar } from '@/platform-admin/lib/pa-avatar';
...
export function PlatformAdminProfilePage() {
  ...
  const save = async () => {
    setLoading(true);
    try {
      await platformAdminApi.updateProfile(form);
  ...
  const onPhotoPick = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      setStoredPaAvatar(user?.userId, dataUrl);
  ...
  const removePhoto = () => {
    setStoredPaAvatar(user?.userId, null);
    ...
    <div className="space-y-6">
      ...
      <div className="neo-card p-6 md:p-8">
        <div className="flex flex-col items-center gap-4 border-b border-[var(--neo-shadow-dark)]/10 pb-6 sm:flex-row sm:items-start">
          <PlatformAdminAvatar userId={user?.userId} name={profile.fullName} size="md" />
          ...
          <div className="neo-form-stack mt-6">
            <div className="neo-inset rounded-2xl p-5">
              ...
              <div className="mt-4 flex justify-center">
                <PlatformAdminAvatar userId={user?.userId} name={form.fullName || profile.fullName} size="md" />
              </div>
            </div>
